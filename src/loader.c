...
static enum ins_opcode_t get_opcode(char * opt) {
	if (!strcmp(opt, OPT_CALC)) {
		...
		return WRITE;
	}else if (!strcmp(opt, OPT_KMALLOC)) {
		return KMALLOC;
	}else if (!strcmp(opt, OPT_KMEM_CACHE_CREATE)) {
		return KMEM_CACHE_CREATE;
	}else if (!strcmp(opt, OPT_KMEM_CACHE_ALLOC)) {
		return KMEM_CACHE_ALLOC;
	}else if (!strcmp(opt, OPT_COPY_FROM_USER)) {
		return COPY_FROM_USER;
	}else if (!strcmp(opt, OPT_COPY_TO_USER)) {
		return COPY_TO_USER;
...
struct pcb_t * load(const char * path) {
	...
	char opcode[32]; /* must hold longest opcode: "kmem_cache_create" = 17 chars */
